...
进入集群详情的「插件管理 → 网络插件」页面。

如果页面显示「切换为统一升级」，请先完成切换。若提示「请先将 IPAMD 插件升级到支持版本」，先在「ipamd 插件」卡片中点击「升级版本」，升级完成后再点击「切换为统一升级」。

![切换为统一升级](/images/network/cni-switch-unified.jpg)

切换后，在「CNI插件升级」卡片中查看当前版本、最新版本及发布时间。有新版本时，点击「升级版本」即可同步更新各节点，无需逐节点操作或分别升级 CNI 和 IPAMD。

![CNI插件升级](/images/network/cni-unified-update.jpg)
...
| 版本    | 类型        | 更新说明                                                     | 发布时间       |
| ------- | ----------- | ------------------------------------------------------------ | -------------- |
| 2.0.7 | Bugfix | 修复固定 IP 资源不可用或创建请求被明确拒绝时可能发生的 IP 泄漏问题 | 2026年9月21日 |
| 2.0.6 | Feature | Pod 独立子网下支持公网出向策略管理（可选NAT网关/节点SNAT） | 2026年8月26日 |
| 2.0.5 | Bugfix | 修复从 v1 升级至 v2 后，节点间 IP 借调（子网 IP 耗尽场景下会发生）可能失败的问题 | 2026年8月10日 |
| 2.0.4 | Feature / Bugfix | 修复 IP 借调后的 GARP/MAC 处理，新增 UNI RPS、辅助 UNI MTU 同步、CNI 容器化部署和 IPAMD 诊断 | 2026年8月7日 |
| 2.0.3 | Feature | Pod 网络 MTU 配置动态适配主机网卡 | 2026年7月7日 |
| 2.0.2 | Bugfix | 修复Pod独立子网模式下Ubuntu系统重启后无法分配IP的问题 | 2026年3月5日 |
| 2.0.1 | Feature | 支持[Pod独立子网功能](/uk8s/network/podnetwork) | 2025年12月29日 |
